...
Modified by Byonghwa Oh (June 12, 2017)
"""
import gzip
import datetime
import sys
...
class ProcessWebscope(object):
    ...
    def parse_text(self, string):
        ...
        if self.python3:
            l = string.decode().split(' ')  # for python 3.6
        else:
            l = string.split(' ')  # for python 2.7

        # get the 2nd best value and its index since feat1 is always 1(maximum)
        ...
        for i in l[10:]:
            if i[0] == '|':
                if bool(temp_dict):  # if we have existent article
        ...
        current_pool_id = 1  # assign temporarily
        if current_pool in self.pool.values():
            ...
            self.session.flush()
            current_pool_id = pool.poolID

            for article_id in current_articles:  # add to db
                self.session.add(self.PoolArticle(poolID=current_pool_id,
                                                  articleID=article_id))
                self.session.flush()

            self.pool[current_pool_id] = current_pool  # update tracked pools

        event = self.Event(datetime=datetime.datetime.fromtimestamp(int(l[0])),
                           poolID=current_pool_id, userID=current_user_id,
                           displayed=int(l[1]), click=int(l[2]))
        self.session.add(event)

        self.session.commit()
        return {'event': event, 'user': user,
                'pool': {'id': current_pool_id, 'articles': current_pool}}
